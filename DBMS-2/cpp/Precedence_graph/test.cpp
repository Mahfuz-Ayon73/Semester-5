...
void findSequence()
{
    vector<vector<int>> tempGraph(matrix.size() + 1);
    ...
    for (int i = 1; i < graph.size(); i++)
    {
        ...
        {
            tempGraph[i].push_back(graph[i][j]);
        }
    }
    ...
    while (!q.empty())
    {
        ...
        for (int i = 0; i < tempGraph[node].size(); i++)
        {
            indegree[tempGraph[node][i]]--;
            if (indegree[tempGraph[node][i]] == 0)
            {
                q.push(tempGraph[node][i]);
            }
        }
    }
...
int main()
{
    ...
    for (int i = 0; i < timeline.size(); i++)
    {
        if (timeline[i].second != "-")
        {
            ...
            {
                if (timeline[i].first == timeline[j].first)
                    continue;
